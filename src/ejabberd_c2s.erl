...
%		AuthModule = xml:get_attr_s(auth_module, Props),
		...
		ejabberd_hooks:run(c2s_auth_result, StateData#state.server,
				   [true, U, StateData#state.server,
				    StateData#state.ip]),
		ejabberd_hooks:run(auth_success, StateData#state.server,
							[U, StateData#state.server, undefined]),
		send_element(StateData,
			    #xmlel{name = <<"success">>,
				    attrs = [{<<"xmlns">>, ?NS_SASL}],
				    children = []}),
...
%		AuthModule = xml:get_attr_s(auth_module, Props),
		...
		ejabberd_hooks:run(c2s_auth_result, StateData#state.server,
				   [false, Username, StateData#state.server,
				    StateData#state.ip]),
		ejabberd_hooks:run(auth_failure, StateData#state.server,
							[Username, StateData#state.server, undefined]),
		send_element(StateData,
			     #xmlel{name = <<"failure">>,
				    attrs = [{<<"xmlns">>, ?NS_SASL}],
				    children =
		...
		send_element(StateData,
			     #xmlel{name = <<"failure">>,
					...
					[#xmlel{name = Error, attrs = [],
						children = []}]}),
		ejabberd_hooks:run(auth_failure, StateData#state.server,
							[undefined, StateData#state.server, undefined]),
		fsm_next_state(wait_for_feature_request, StateData)
	  end;
      _ ->
	  process_unauthenticated_stanza(StateData, El),
